...
const actions = {
  ...
  REMOVE_LISTEN_BY_OBJECT_GROUP: type + "REMOVE_LISTEN_BY_OBJECT_GROUP",
  ...
  UPDATE_ARRAY: type + "UPDATE_ARRAY",

  readFromDatabase: ({ startTime, endTime }) => {
    return {
  ...
  listenByObjectGroupSuccess: (data) => ({
    type: actions.READ_BY_OBJECTGROUP_SUCCESS,
    payload: { data },
  }),

  listenByObjectGroupError: (error) => ({
    type: actions.READ_BY_OBJECTGROUP_ERROR,
    payload: { error },
  }),

  removeListenByObjectGroup: () => {
    return { type: actions.REMOVE_LISTEN_BY_OBJECT_GROUP };
  },
